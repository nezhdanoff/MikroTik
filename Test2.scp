...
:if ($isUP=true) do={
	...
	:if ($ifstatus="down") do={
		...
		:if ($isLOG) do={:log info "Link UP. Power is ON ($globalDiff)"};
		$UpdateDate1;
		}
	:set $ifstatus "up"
	} else={
		...
		:if ($isLOG) do={:log info "Link DOWN. Power is OFF ($globalDiff)"};
		$UpdateDate1;
		}
	:set $ifstatus "down"
	}
...
:if ($isLOG) do={
	:log info  "before function UpdateDate2 date2 = $date2"
	$UpdateDate2;
	:log info  "after function UpdateDate2 date2 = $date2"
	/system script run diffDate;
	:if ($isUP) do={ 
